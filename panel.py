...
class _PlainFormatter(logging.Formatter):
    def _sanitize_value(self, v: Any) -> Any:
        if isinstance(v, str) and len(v) > 200:
            stripped: str = v.replace("\n", "").replace("\r", "").replace(" ", "")
            if stripped.isascii() and all(c.isalnum() or c in "+/=" for c in stripped[:64]):
...
def _handle_server_error(request: bytes, client_address: tuple[str, int]) -> None:
